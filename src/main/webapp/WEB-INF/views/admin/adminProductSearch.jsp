...
<title>관리자 상품관리 검색</title>
	...
	<script>
		function selChange() {
			...
			location.href="adminproductsearch.bill?selection=${selection}&adminProductSearchText=${adminProductSearchText}&nowPage=${paging.nowPage}&cntPerPage="+sel;
		}
	</script>
...
<!-- 페이징 -->
	...
	<div style="display: block; text-align: center;">		
		<c:if test="${paging.startPage != 1 }">
			<a href="adminproductsearch.bill?selection=${selection }&adminProductSearchText=${adminProductSearchText}&nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="adminproductsearch.bill?selection=${selection }&adminProductSearchText=${adminProductSearchText}&nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="adminproductsearch.bill?selection=${selection }&adminProductSearchText=${adminProductSearchText}&nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
		</c:if>
	</div>
